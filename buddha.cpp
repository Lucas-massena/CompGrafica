...
void main() {
    ...
    vec3 baseTex = texture(tex0, uvMap(P * 4.0)).rgb;
    vec3 buddhaColor = vec3(1.0, 0.1, 0.1);
    vec3 texturedColor = mix(buddhaColor, baseTex, 0.8);
    vec3 col = 0.1 * texturedColor + diff * texturedColor + spec * vec3(1.0,1.0,1.0);
...
GLuint CreateFurTexture(int size = 512) {
    ...
    for (int y = 0; y < size; y++)
        for (int x = 0; x < size; x++) {
            ...
            unsigned char r = checker ? 255 : 0;
            unsigned char g = checker ? 255 : 0;
            unsigned char b = checker ? 0 : 255;
...
int main() {
    ...
    GLFWwindow* w = glfwCreateWindow(1200, 1000, "Buddha", nullptr, nullptr);
    ...
    std::vector<Vertex> verts;
    std::vector<unsigned> idx;
    LoadOBJ("models/buddha.obj", verts, idx);
    ...
    while (!glfwWindowShouldClose(w)) {
        ...
        model = Mul(RotateY(-3.1415f / 2), model);
